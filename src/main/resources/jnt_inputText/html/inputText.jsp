...
<input ${disabled} type="text" ${required} class="${required}" id="${currentNode.name}" name="${currentNode.name}" maxlength="${currentNode.properties.maxLength.long}" size="${currentNode.properties.size.long}"
                   value="<c:if test="${not empty sessionScope.formError or not empty requestScope.isWebflow}">${sessionScope.formDatas[currentNode.name][0]}</c:if><c:if test="${empty currentNode.properties.mask and empty sessionScope.formError and requestScope.isWebflow}">${currentNode.properties.defaultValue.string}</c:if>"/>
