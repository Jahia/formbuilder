...
<select ${disabled} name="${currentNode.name}"  ${required} class="${required}">
    <c:forEach items="${jcr:getNodes(currentNode,'jnt:formListElement')}" var="option">
        <option value="${option.name}" <c:if test="${not empty sessionScope.formError and param['ec'] eq form.identifier  and sessionScope.formDatas[currentNode.name][0] eq option.name}">selected="true"</c:if>>${option.properties['jcr:title'].string}</option>
    </c:forEach>
</select>
