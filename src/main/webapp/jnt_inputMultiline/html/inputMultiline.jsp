...
<p class="field">
    ...
    <textarea ${disabled} type="text" ${required} class="${required}" name="${currentNode.name}" cols="${currentNode.properties['cols'].string}" rows="${currentNode.properties['rows'].string}"><c:if test="${not empty sessionScope.formError and param['ec'] eq form.identifier }">${sessionScope.formDatas[currentNode.name][0]}</c:if><c:if test="${empty sessionScope.formError}">${currentNode.properties['defaultValue'].string}</c:if>
    </textarea>
