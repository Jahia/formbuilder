...
<c:forEach items="${jcr:getNodes(currentNode,'jnt:formListElement')}" var="option">
    <input ${disabled} type="radio" ${required} class="${required}" name="${currentNode.name}" id="${currentNode.name}" value="${option.name}" <c:if test="${not empty sessionScope.formError and param['ec'] eq form.identifier and sessionScope.formDatas[currentNode.name][0] eq option.name}">checked="true"</c:if>/>
